...
(all video qualities), podcasts, albums and playlists on Radiojavan.com. Also, it generates file size beside depicting cover photos and 
...
def Image(html):
    ...
    if a2<a1:
        a2=html.rfind('<meta property="og:')
    a3=html[a1+len('<link rel="image_src" href="'):a2-2]
    a4=re.match(r'(.*)"/>', a3, re.M|re.I)
    
    image_name=artist_song(html)[1]
    if artist_song(html)[1]=='RadioJavan':
        image_name="%s_%s"%(artist_song(html)[1],artist_song(html)[0]) 
    ...
    try:
        ...
        with open('../RJ/tmp/%s_b_image.jpg'%image_name, 'wb') as f:
            f.write(image_big_dl.content)
    image_big='http://mynext.pro/RJ/tmp/%s_b_image.jpg'%image_name
    A=[]
    A.append(image_big)
    ...
    if float(file_size(i_small)[0])!=0:
        ...
        with open('../RJ/tmp/%s_s_image.jpg'%image_name, 'wb') as f:
            f.write(image_small_dl.content)
        image_small='http://mynext.pro/RJ/tmp/%s_s_image.jpg'%image_name
    else:
...
def artist_song(Html):
    ...
    a3=Html[a1+1+len('<meta property="og:title" content='):a2-5]
    if (a3.find('"')>0):
        a4_fck=a3[:a3.find('"')]
    else:
        a4_fck=a3
...
def playlist(URL,HTML):
    i=0
    track_list=["Null"]

#https://www.radiojavan.com/mp3s/mp3/SheryM-Roozamun-(Remix)?playlist=02596ef9986a&index=0
#https://www.radiojavan.com/mp3s/playlist_start?id=02596ef9986a&index=6
#https://www.radiojavan.com/playlists/playlist/mp3/02596ef9986a
    
    if (URL.find('/playlists/playlist/'))<0:
        a20=URL.find('?playlist=')
        a21=URL.find("/playlist_start?id=")
        a22=URL.find('&index=')
        playlist_no=(HTML.count("&amp;index=")//2)-1

        if a20>0:
            playlist_ID=URL[a20+len('?playlist='):a22]
            URL2=l_playlist+playlist_ID;
        elif a21>0:
            playlist_ID=URL[a21+len("/playlist_start?id="):a22]
            URL2=l_playlist+playlist_ID;
        ...
        URL2=URL
        HTML2=HTML
    a1=HTML2.find('songs</span> | <span id="follower_count">')
    playlist_no=int(HTML2[a1-3:a1-1])
    a30=URL2.find('mp3/');
    playlist_ID=URL2[a30+len('mp3/'):]
    if URL2.rfind('/')>(a30+len('mp3/')):
        playlist_ID=URL2[a30+len('mp3/'):URL2.rfind('/')]

    #print(playlist_no);
    #print(playlist_ID);
    #print(URL2); 
            
    HTML3=requests.get("http://www.radiojavan.com/mp3s/playlist_start?id="+playlist_ID+"&index="+"%d"%i).text
    b=HTML3[HTML3.find('<span class="song_name">'):HTML3.rfind('<span class="song_name">')]
    b_len=len('<span class="song_name">')
    iter=re.finditer(r'<span class="song_name">', b)
    indices=[m.start(0) for m in iter]
    #print("indices: %s"%indices)
    #for in range (0,playlist_no):
    while i<playlist_no:
        track_list.append("http://www.radiojavan.com/mp3s/playlist_start?id="+playlist_ID+"&index="+"%d"%i)
...
if (url.find('radiojavan.com'))>=0:
    ...
    if (url.find('playlist'))>0:
        a='%s'%(list_pr(list_DL(playlist(url,html)),track_name(playlist(url,html))))
    
    elif (url.find('/mp3s/mp3/'))>0:
        a='%s'%single_pr(mp3(url))
    
    elif (url.find('/videos/video/'))>0:
        a='%s'%vid_pr(video(url))
        
    elif (url.find('/mp3s/album/'))>0:
        a='%s'%(list_pr(list_DL(album(url)),track_name(album(url)))) 
        
    elif (url.find('/podcasts/podcast/'))>0:
