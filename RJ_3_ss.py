...
def list_DL(ID_List):
    ...
    while i < len(ID_List):
        List.append(check_host(l_mp3[:4],ID_List[i])[0])
        i+=1
    return(List)
...
def list_pr(list_pr):
    ...
    if ((album(url)[0]).isdigit() == True):
        print('<table><tr><td>'+'</br><a href="%(1)s">Download track %(2)s</a> (%(3)s) at: %(1)s'%{'1':list_DL(list_ID(album(url)))[int(album(url)[0])],'2':(int(album(url)[0]))+1,'3':file_size(list_DL(list_ID(album(url)))[int(album(url)[0])])[1]}+'</br></br></br>Other album tracks are:'+'</td></tr>')
...
if (url.find('radiojavan.com'))>=0:
    ...
    if (url.find('/mp3/'))>0:
        ...
        a='%s'%(list_pr(list_DL(list_ID(album(url)))))  
